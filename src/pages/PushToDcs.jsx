...
function PushToDcs({ repoPath, repoName, open, closeFn }) {
  ...
  const pushRepo = async (repo_path, username, password) => {
    ...
    if (pushResponse.ok) {
      enqueueSnackbar(doI18n("pages:content:push_complete", i18nRef.current), {
        variant: "success",
      });
    } else {
      enqueueSnackbar(doI18n("pages:content:could_not_push", i18nRef.current), {
        variant: "error",
      });
    }
  };
    ...
    >
      ...
      <DialogActions>
        ...
        <Button
          ...
          onClick={() => {
            pushRepo(repoPath, dcsUsername, dcsPassword).then();
            closeFn();
          }}
        >
